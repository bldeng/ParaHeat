...
void GeodesicDistanceSolver::prepare_integrate_geodesic_distance()
{
    ...
    OMP_PARALLEL
    {
        ...
        OMP_SINGLE
        {
            ...
            primal_residual_sqr_norm_threshold = param.grad_solver_eps * param.grad_solver_eps;
            dual_residual_sqr_norm_threshold   = param.grad_solver_eps * param.grad_solver_eps;
            (*prev_SX) = (*current_SX);
        }
...
void GeodesicDistanceSolver::update_X()
{
    OMP_FOR
    for(int i = 0; i < n_edges; ++ i)
    {
...
void GeodesicDistanceSolver::update_dual_variables()
{
    ...
    OMP_SINGLE
    {
        need_compute_residual_norms = ((iter_num + 1) % param.grad_solver_convergence_check_frequency == 0);
    }
    ...
    OMP_SECTIONS
    {
        ...
        OMP_SECTION
        {
            if(need_compute_residual_norms)
            {
                dual_residual_sqr_norm = ((*current_SX) - (*prev_SX)).squaredNorm() * param.penalty * param.penalty;
            }
        }
